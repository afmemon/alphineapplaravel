<?php

namespace App\Http\Controllers;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;

class EmployeesController extends Controller
{
    public function index()
    {
        $employees = collect([
            [
                'name' => 'John',
                'email' => 'john3@example.com',
                'sales' => [
                    ['customer' => 'The Blue Rabbit Company', 'order_total' => 7444],
                    ['customer' => 'Black Melon', 'order_total' => 1445],
                    ['customer' => 'Foggy Toaster', 'order_total' => 700],
                ],
            ],
            [
                'name' => 'Jane',
                'email' => 'jane8@example.com',
                'sales' => [
                    ['customer' => 'The Grey Apple Company', 'order_total' => 203],
                    ['customer' => 'Yellow Cake', 'order_total' => 8730],
                    ['customer' => 'The Piping Bull Company', 'order_total' => 3337],
                    ['customer' => 'The Cloudy Dog Company', 'order_total' => 5310],
                ],
            ],
            [
                'name' => 'Dave',
                'email' => 'dave1@example.com',
                'sales' => [
                    ['customer' => 'The Acute Toaster Company', 'order_total' => 1091],
                    ['customer' => 'Green Mobile', 'order_total' => 2370],
                ],
            ],
        ]);

        $result = $employees->flatMap(function ($employee) {
            return collect($employee['sales'])->map(function ($sale) use ($employee) {
                return [
                    'employee' => $employee['name'],
                    'customer' => $sale['customer'],
                    'order_total' => $sale['order_total'],
                ];
            });
        })->sortByDesc('order_total')->first();

        return  view("livewire.employee", compact("result"));
    }


    public function rankingRecords()
    {
        $scores = collect([
            ['score' => 76, 'team' => 'A'],
            ['score' => 62, 'team' => 'B'],
            ['score' => 82, 'team' => 'C'],
            ['score' => 86, 'team' => 'D'],
            ['score' => 91, 'team' => 'E'],
            ['score' => 67, 'team' => 'F'],
            ['score' => 67, 'team' => 'G'],
            ['score' => 82, 'team' => 'H'],
        ]);


        $rr = [];
        $rankedScores = collect($scores)->sortByDesc('score')->groupBy('score')->values();

        $rankedTeams = $rankedScores->map(function ($group) {
            return collect($group)->pluck('team')->toArray();
        })->flatMap(function ($teams, $rank) use ($rankedScores) {
            $nextRank = $rank + 1;
            if ($rankedScores->has($nextRank)) {
                $teamCount = count($teams);
                $nextRankCount = count($rankedScores[$nextRank]);
                if ($teamCount === $nextRankCount) {
                    return $teams;
                }
            }

            return collect($teams)->map(function ($team) use ($rank) {
                return [
                    'team' => $team,
                    'rank' => $rank + 1,
                ];
            });
        })->values();
        // foreach ($rankedTeams as $team) {
        //     echo "Team: " . $team['team'] . ", Rank: " . $team['rank'] . "\n";
        // }
        echo "<pre>";
        print_r($rankedTeams);
    }
}
